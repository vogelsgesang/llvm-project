...
* The switch-resume coroutine frame layout now places the promise at exactly
  `2*sizeof(void*)` from the coroutine handle, even when the promise has
  alignment greater than `2*sizeof(void*)`. Previously the promise was placed
  at `alignTo(2*sizeof(void*), alignof(promise))`, which violated the C++20
  coroutine ABI (Itanium draft) and broke ABI-aware consumers of
  `coroutine_handle::promise()` for over-aligned promises. For such
  coroutines, LLVM now inserts `alignof(promise) - 2*sizeof(void*)` bytes of
  padding before `resume_fn`; the handle still points at `resume_fn`.
  Coroutines whose promise has alignment ≤ `2*sizeof(void*)` (the
  overwhelming majority of real-world cases) are unaffected and produce
  byte-identical IR. The new `"coro-legacy-promise-layout"` function
  attribute opts each coroutine and `__builtin_coro_promise` caller into the
  pre-fix layout for cross-TU compatibility with code generated by
  older releases; Clang sets it automatically when
  `-fclang-abi-compat <= 21` is in effect. CoroElide currently bails out
  for over-aligned coroutines; restoring heap-allocation elision for them
  is a follow-up. See issue
  [#58397](https://github.com/llvm/llvm-project/issues/58397).
