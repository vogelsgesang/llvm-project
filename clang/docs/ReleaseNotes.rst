...
- The C++20 coroutine promise is now always placed at offset ``2*sizeof(void*)``
  from the coroutine handle, matching the C++20 coroutine ABI (Itanium draft,
  also adopted by MSVC). Previously the promise was placed at
  ``alignTo(2*sizeof(void*), alignof(promise))``, which moved it to a higher
  offset for promises whose alignment exceeded ``2*sizeof(void*)`` and broke
  cross-TU users of ``std::coroutine_handle::promise()`` /
  ``from_promise()`` for such promises. This is an ABI break that only
  affects coroutines whose promise has ``alignas`` greater than
  ``2*sizeof(void*)``; promises with the default ABI alignment are
  unaffected. Pass ``-fclang-abi-compat=22`` (or lower) to keep the previous
  (broken) layout when mixing object files with code generated by Clang 22
  or older.

  When using ``-fclang-abi-compat=22`` as an opt-out, **every translation
  unit in the linked binary that either defines such a coroutine or
  instantiates ``std::coroutine_handle<P>::promise()`` /
  ``from_promise()`` for the same promise type must use the same compat
  setting.** A mixed-flag build silently miscompiles: each TU bakes the
  promise offset (``2*sizeof(void*)`` vs ``alignTo(2*sizeof(void*),
  alignof(promise))``) into its own copy of the inline accessor at
  CodeGen time, the linker (or LTO) picks one ``linkonce_odr``
  instantiation arbitrarily, and the offset baked into the surviving
  copy may disagree with the layout of the handle it is called on.
  Clang does **not** emit a diagnostic for this mismatch. If you cannot
  guarantee a uniform setting across all TUs (including third-party
  libraries and the C++ standard library), prefer the default (fixed)
  layout. See `LLVM issue #58397
  <https://github.com/llvm/llvm-project/issues/58397>`_.
