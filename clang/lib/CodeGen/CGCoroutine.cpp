...
void CodeGenFunction::EmitCoroutineBody(const CoroutineBodyStmt &S) {
  ...
  // ABI compat: under `-fclang-abi-compat <= 22` we lay out the coroutine
  // frame using the pre-LLVM-fix promise placement (promise at
  // `alignTo(2*ptrsize, alignof(promise))` instead of at the ABI-required
  // `2*ptrsize`). The legacy layout is silently UB against the C++20
  // coroutine ABI for over-aligned promises but preserves link
  // compatibility with code generated by Clang 22 and older releases. See
  // LLVM issue #58397 and `clang/include/clang/Basic/ABIVersions.def`.
  if (getLangOpts().getClangABICompat() <= LangOptions::ClangABI::Ver22)
    CurFn->addFnAttr("coro-legacy-promise-layout");
...
RValue CodeGenFunction::EmitCoroutineIntrinsic(const CallExpr *E,
  ...
  else if (IID == llvm::Intrinsic::coro_promise) {
    // `coroutine_handle::promise()` / `from_promise()` ultimately call
    // `__builtin_coro_promise` from the *caller's* TU, which can differ
    // from the coroutine's TU. We must compute the same offset on both
    // sides, so we propagate the same legacy attribute that
    // EmitCoroutineBody sets on the ramp. `CoroEarly::lowerCoroPromise`
    // reads it off `Intrin->getFunction()`.
    if (getLangOpts().getClangABICompat() <= LangOptions::ClangABI::Ver22)
      CurFn->addFnAttr("coro-legacy-promise-layout");
  }
